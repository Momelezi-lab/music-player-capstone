...
function App() {
  ...
  /**
   * EFFECT: Save playlists to localStorage
   */
  useEffect(() => {
  ...
  /**
   * EFFECT: Close add to playlist dropdown when clicking outside
   */
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (showAddToPlaylist && !e.target.closest('[data-playlist-dropdown]')) {
  ...
  /**
   * showError - Shows error/success message to user
   */
  ...
  const searchMusic = async () => {
    ...
    try {
      ...
      if (!response.data || !response.data.data || response.data.data.length === 0) {
        setTracks([]);
        showError("No results found. Try a different search term.", "error");
        return;
      }
      ...
      let errorMsg = "Failed to search. Please check your connection and try again.";
  ...
  // ============================================================================
  // PLAYLIST FUNCTIONS
  // These functions manage custom playlists
  // ============================================================================
  ...
  const createPlaylist = () => {
    if (!newPlaylistName.trim()) {
      showError("Please enter a playlist name", "error");
      return;
    }
    ...
    if (playlists.some((p) => p.name.toLowerCase() === trimmedName.toLowerCase())) {
      showError("A playlist with this name already exists", "error");
      return;
    }
    ...
    showError(`Playlist "${trimmedName}" created successfully!`, "success");
  };

  /**
   * deletePlaylist - Deletes a custom playlist
   */
      ...
      prev.map((playlist) => {
        if (playlist.id === playlistId) {
          ...
          if (playlist.tracks.some((t) => t.id === track.id)) {
            showError("This song is already in the playlist", "error");
            return playlist;
          }
          const playlistName = playlist.name;
          setTimeout(() => showError(`Added to "${playlistName}"`, "success"), 100);
          return {
            ...playlist,
  ...
  const removeTrackFromPlaylist = (playlistId, trackId) => {
    ...
    showError("Song removed from playlist", "success");
  };

  /**
   * getPlaylistTracks - Gets tracks for a selected playlist
   */
  ...
  // Get all songs to display (playlist tracks, search results, or trending)
                ...
                >
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "12px", flex: 1, minWidth: 0 }}>
                      ...
                      >
                        <FaMusic style={{ fontSize: "16px", color: "white" }} />
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p
                          style={{
                  ...
                  >
                    ...
                    {selectedPlaylist ? (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          removeTrackFromPlaylist(selectedPlaylist, track.id);
                        }}
                        style={{
                          background: "rgba(239, 68, 68, 0.2)",
                          border: "none",
                      ...
                      <>
                        <div style={{ position: "relative" }}>
                          ...
                          {showAddToPlaylist === track.id && playlists.length > 0 && (
                            <div
